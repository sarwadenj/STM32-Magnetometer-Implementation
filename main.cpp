...
/*parking logic operators*/
static int status=3;
static int statecount=0;
static int id=1;
/************************************Magnetometer Initilization***********************************************************/
/*MLX*/
DigitalOut led(LED1);
 ...
 //       printf("\r\n No sensor found \r\n");
   ...
   /*form packet to send*/
     packet_len = sprintf((char *) tx_buffer, "{\"X\":%hd,\"Y\":%hd,\"Z\":%hd,\"status\":%d}",magnetic_field[0], magnetic_field[1], magnetic_field[2],status);
       ...
       printf("%s\n", tx_buffer);
     /*logic: difference in Z axis and threshold value*/
      if(magnetic_field[2] >= threshold_values[2]+200 || magnetic_field[2] <= threshold_values[2]-200)
       {
           status=1;
 //          prestatus=status;
           printf("Parking FULL\n");
        ...
        {
            status=0;
            printf("Parking Slot Available\n");
 
            }
            /*check status change and send packet*/
         if(status==1 && statecount==1)
         {
             retcode = lorawan.send(MBED_CONF_LORA_APP_PORT, tx_buffer, packet_len,
                           MSG_UNCONFIRMED_FLAG);
             printf("PARKING STATE CHANGE\n");
             statecount=0;
             }
        if(status==0 && statecount==0)
        {
            retcode = lorawan.send(MBED_CONF_LORA_APP_PORT, tx_buffer, packet_len,
                           MSG_UNCONFIRMED_FLAG);
             printf("PARKING STATE CHANGE\n");
